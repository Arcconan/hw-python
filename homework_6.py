    ...
    for element in folder_path.iterdir():
        if element.is_dir() and element not in target_folders:
            ...
            for file in folder_path.iterdir():
                if file.suffix[
                   1:].upper() in image_ext:  # переводимо розширення у верхній регістр і перевіряємо наявність в image_ext
                    image_folder = Path('images')
                    file.replace(
                        sort_folder / image_folder / normalize(file.name))  # переміщаємо до папки images змінивши імя
